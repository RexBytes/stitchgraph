...
**v3.15.0 adds C#**, and **v3.16.0 adds Ruby**, so the statement layer now covers **Python + the JS
family (js/ts/tsx) + Go + Rust + C/C++ + Java + C# + Ruby**; the remaining tree-sitter languages
(PHP, Bash) are the rest of the sweep. All on-demand (no persisted deep edges —
