...
**v3.15.0 adds C#**, so the statement layer now covers **Python + the JS family (js/ts/tsx) + Go +
Rust + C/C++ + Java + C#**; the remaining tree-sitter languages (Ruby, PHP, Bash) are the rest of the sweep. All on-demand (no persisted deep edges —
