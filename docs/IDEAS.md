...
**v3.11.0 adds Go**, **v3.12.0 adds Rust**, **v3.13.0 adds C/C++**, and **v3.14.0 adds Java**, so the
statement layer now covers **Python + the JS family (js/ts/tsx) + Go + Rust + C/C++ + Java**; the
remaining tree-sitter languages (C#, Ruby, PHP, Bash) are the rest of the sweep. All on-demand (no persisted deep edges —
