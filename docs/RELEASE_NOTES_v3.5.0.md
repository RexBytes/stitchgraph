...
- ruff + mypy clean; full suite **912** passing; differential oracle suite **278**.
- New **C/C++ body-matrix completeness oracle** (`tests/oracles/test_structure_cpp_completeness.py`):
  ...
  `sizeof` is a compile-time constant, nested lambda opaque). The oracle caught a real drop during
