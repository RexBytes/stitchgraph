...
| Item | Effort | Status |
|---|---|---|
| ~~External field-review answers~~ | — | **Done, v3.51.0** — eight requests from the first external agent field review (Opus 4.8 on a 40k-line Rust crate) shipped: actionable LSP diagnostics (rustup-shim detection), heuristic-cycle suppression, tiered/ranked/capped `impact_of`, bounded MCP output, stable `review_codes`, `find_hotspots` convergence, IDF mode labels, coverage id-drift reconciliation at the load boundary. The batch itself went through two adversarial self-review rounds (panels R287–R288: 21 findings fixed, incl. 2 regressions round 1 introduced), a dogfood pass (test-owned stubs greened), and an independent agent review (quoted-config-bool guard, lean-import + adapter-parity pins). |
