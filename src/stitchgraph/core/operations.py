...
def _pdg_for_node(store: Store, node) -> tuple[list[str], list] | None:
    ...
    stdlib ast), the JS family (js/ts/tsx), Go, Rust, and C/C++ (tree-sitter) so far; other languages
    are a future sweep. Selects the frontend by extension and the function by the qualname in the id."""
    from pathlib import Path
    ...
    for mod in (structure_js, structure_go, structure_rust, structure_cpp):
        lang = mod._lang_for_ext(suf)
        if lang is not None:
...
def _body_matrix(store: Store, scope: str, layer: str) -> Result:
    ...
    if layer == Layer.STATEMENT.value:
        from . import structure_cpp, structure_go, structure_js, structure_rust
...
def get_matrix(store: Store, scope: str, relation: str = "CALLS",
    ...
    dependence — Python + the JS family + Go + Rust + C/C++ so far); "expression" drills into its intra-procedural
