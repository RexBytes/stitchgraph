...
### Added
- **Turnkey coverage across the board** (research/26) — the LLM field
  review's setup-friction finding, generalized and closed: it was never
  just Rust; every language but Python shipped a TODO template.
  `scaffold_coverage` now emits RUNNABLE per-test capture loops for
  **Rust** (cargo-llvm-cov: one instrumented build, then ~0.5 s per test),
  **Go** (per-test `-coverprofile -coverpkg`), and **JS/TS** (jest/vitest,
  per test FILE — documented honestly). The design insight that makes the
  converters trivial: the kit is generated by stitchgraph, whose index
  already holds every function's span — so each kit ships `spans.json` and
  one stdlib-only converter maps covered lines → innermost enclosing span
  → node-id-EXACT function ids, with no language parser and no stitchgraph
  inside the sandbox. Java remains the one honest template (JaCoCo
  per-test attribution needs build-system integration).
  Field-validated end to end: the generated Rust kit ran fd's 267 tests
  unedited and `find_modes` answered (intrinsic dimensionality 7, 16
  modes, minimal test set 154 of 267); Go and both JS runners produce
  exact attribution on fixture projects.
