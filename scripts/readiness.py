#!/usr/bin/env python3
"""Compute a package's Release-Readiness Score (RRS) and convergence.
...
def run_gates() -> tuple[dict[str, bool], float]:
    ...
    rc_pytest, _ = _run(
        [
            ...
            f"--cov={pkg}", f"--cov-report=json:{cov_json}",
        ]
    )
    coverage = 0.0
    if cov_json.exists():
...
def main() -> int:
    ...
    # --- report ---
    print("=" * 56)
    print(f" {cfg.get('package', 'package')} Release Readiness")
    print("=" * 56)
    print("Gates:")
