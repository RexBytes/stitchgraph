...
- **How do two builds differ?** — `graph_diff`: call-level deltas **plus** body-shape changes
  (Python + JS/TS/TSX + Go + Rust), so a data-flow bug that leaves the call graph identical still shows up.
...
- **Sees inside functions, not just between them.** The **body matrix**
  (Python + JS/TS/TSX + Go + Rust) fingerprints each function's value flow order- and
...
## Status (v3.4.0 — intra-procedural body matrix for Python + JS/TS/TSX + Go + Rust)
...
and Rust (v3.4.0). See [`docs/STATUS.md`](docs/STATUS.md) for the full table + roadmap.

### Headline: the intra-procedural body matrix (Python + JS/TS/TSX + Go + Rust)
...
and **v3.4.0 adds Rust**: a per-function **value-flow fingerprint** (`core/structure.py`
for Python, `core/structure_js.py` for the JS family, `core/structure_go.py` for Go,
`core/structure_rust.py` for Rust) — operations + control points, data + control
...
rule is structurally unaffected. **Python is stdlib-only; the JS/TS, Go, and Rust
layers need the tree-sitter extra.** Cross-language *body* comparison stays
...
limits in [`docs/RELEASE_NOTES_v3.4.0.md`](docs/RELEASE_NOTES_v3.4.0.md).
...
with sg.Store("stitchgraph.db") as store:
    ...
    # rank stored functions by body shape (renamed/reordered clones; Python, JS/TS, Go, or Rust)
...
- **Intra-procedural body matrix (Python v3.0.0; JS/TS/TSX v3.2.0; Go v3.3.0; Rust v3.4.0)** — a
  ...
  read-only (never feeds `find_stale`); ranks/diffs within one language. The JS/TS, Go, and Rust
  layers need the tree-sitter extra; Python is stdlib-only.
...
| `find_similar` | What else looks like this (duplication / refactor targets)? — token (default) or `mode="structure"` body-shape (Python + JS/TS/TSX + Go + Rust). |
| `graph_diff` | How do two indexes differ (translation fidelity / plan-vs-actual)? Call-level deltas + body-shape changes (Python + JS/TS/TSX + Go + Rust). |
