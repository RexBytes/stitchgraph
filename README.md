...
- **Layered — one graph, pick the depth (§5c).** `get_matrix(layer="call")` is the
  ...
  function's program-dependence graph (control/data deps, Python + the JS family + Go + Rust for now);
...
## Status (v3.12.0 — layered code-property graph: call ↔ statement ↔ expression drill-down over the 12-language body matrix)
