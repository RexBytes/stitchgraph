...
| R288 | 1 (self) | ✗ | Round 2: the SAME 8-angle process turned on R287's own fixes — the confirmation-gate principle (panel S's lesson) applied to a self-review. 10 findings survived, 2 of them regressions R287 itself introduced: (1) the get_matrix MCP exemption reopened the unbounded-blob hole (its "op-layer self-bound" is the caller-supplied `limit`, no ceiling; statement/expression layers bypass the member check entirely) — replaced with a correlated cut: labels trimmed to the budget, cells kept only where both endpoints survive, alignment preserved and both cuts reported; (2) moving the codes backfill out of to_dict traded the chokepoint guarantee for a hook that only watches the `needs_review` name — `review_codes.clear()` on a flagged Result serialized an unfilterable envelope (reproduced live); to_dict self-heals again via one shared backfill method. Also: refuse(result=[]) coded HEDGED_RESULT for an EMPTY payload (now REFUSED — an empty advisory is "retry differently"); impact_of's `any_ambiguous` tallied over ALL backing edges so a redundant AMBIGUOUS edge onto a confident dependent flipped the envelope provenance (now scoped to ambiguous-tier sources, which is exact — a node with an EXTRACTED edge to a confident node would itself be confident); the edges_all scan ran even when nothing consumed it (now skipped/early-broken; huge radii keep plain sorts); edge-budget degradation was silent (now meta.distances_skipped); the diagnosis memo served stale verdicts after in-place binary fixes and cached transient timeouts (mtime in key; transients never cached); find_chokepoints' leftover limit idiom accepted limit=True as 1 (_pos_int); find_similar/get_matrix accepted limit<=0 (now refuse like their sibling); find_hotspots' two drifted refusal messages collapsed into one honest builder. Refuted: the __setattr__ per-write cost (Results are one-per-op-call — noise), the tier-gate engine-consistency worry (confident ⊆ dependents by construction on all three engines). DOGFOOD: reindexed stitchgraph itself (3,006 nodes, `--precise`) and ran the static battery — the new features held (5 heuristic cycles suppressed with honest meta; hotspots = operations/store/extract with tests excluded; impact_of split 2,536 dependents into 227 confident / 2,309 ambiguous; find-stale's 13 candidates all genuinely-dead research/ fixtures) and found one issue every panel missed: the repo's sole RED was a deliberately-empty fake in tests/test_mcp.py — scan's stub detector lacked the god-object test-ownership exclusion. Test-owned stubs are now GREEN advisories, pinned by test. |
